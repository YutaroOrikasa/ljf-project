...
namespace detail {
    ...
    constexpr auto sep_many_optsep(Parser p, SepParser sep) {
        ...
        return parser::Parser([parser](auto &&token_stream) {
            using namespace parser;

            using vec_value_ty =
                result_content_t<Parser, decltype(token_stream)>;
            ...
            while (true) {
                ...
                // (p sep) (p sep) ... (p sep); ends_with_sep=true were
                // parsed or nothing; ends_with_sep=false  were parsed.
    ...
    /// result type: std::tuple{vector<result of p>, bool:ends_with_sep,
    /// std::optional<result of end>}
    template <typename Parser, typename SepParser, typename EndParser>
    constexpr auto sep_many_optsep_optend(Parser p, SepParser sep,
                                          EndParser end) {
        ...
        return parser::Parser([=](auto &&token_stream) {
            ...
            using vec_value_ty =
                result_content_t<Parser, decltype(token_stream)>;
            using end_result_ty =
                result_content_t<EndParser, decltype(token_stream)>;
            using vec_ty = std::vector<vec_value_ty>;
            using tuple_ty =
                std::tuple<vec_ty, bool, std::optional<end_result_ty>>;
            ...
            while (true) {
                ...
                if (var.index() == 1) {
                    // `end' matched
                    return ResultTy(
                        {std::move(vec), ends_with_sep, std::get<1>(var)});
                }
