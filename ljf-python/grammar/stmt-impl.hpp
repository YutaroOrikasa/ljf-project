...
namespace detail {
    ...
    static constexpr auto fold_assign = [](auto &&first,
                                           auto &&right_list) -> Stmt {
        if (right_list.empty()) {
        ...
        return AssignStmt{std::move(left_hand_side_list),
    ...
    template <typename T, typename P> auto parser_result_type(P &&parser) {
        return result_type<T> <<= parser;
    }

    template <typename T, typename P> auto parser_result_brace(P &&parser) {
        return result_type<T> <<= brace_init <<= parser;
    }

    namespace impl {

        template <typename T> std::optional<T> flatten(std::optional<T> opt) {
            return opt;
        }
...
template <class TokenStream> inline StmtGrammars<TokenStream>::StmtGrammars() {
    ...
    // #       file_input is a module or sequence of commands read from an
    // input file; #       eval_input is the input for the eval() functions.
    ...
    // decorator = "@"_p + dotted_name + opt["("_p + opt[E::arglist] + ")"]
    // + NEWLINE; decorators = decorator * _many1; decorated = decorators +
    // (classdef | funcdef | async_funcdef); async_funcdef = "async"_p +
    // funcdef;

    // Simplified funcdef definition (return type annotation is omitted)
    funcdef = brace_init <<= "def"_sep + NAME + parameters + ":"_sep + suite;
    // funcdef = "def"_sep + NAME + parameters + opt["->"_sep + E::test] +
    // ":"_sep + suite;

    parameters = "("_sep + opt[typedargslist] + ")"_sep;
    const Parser defparameter = result_type<DefParameter> <<= brace_init <<=
        tfpdef + opt["="_sep + E::test];
    const Parser starred_param = result_type<StarredParameter> <<=
        brace_init <<= "*"_sep + tfpdef;

    const Parser stars = result_type<StarredParams> <<=
        parser_result_brace<ast::StarredParameter>("*"_sep + tfpdef) //
        + opt[","_sep + parser_result_brace<ast::DoubleStarredParameter>(
                            "**"_sep + tfpdef)];
    ...
    // varargslist = (vfpdef + opt["="_p + E::test] + (","_p + vfpdef +
    // opt["="_p + E::test]) * _many + opt[","_p + opt["*"_p + opt[vfpdef] +
    // (","_p + vfpdef + opt["="_p + E::test]) * _many + opt[","_p + "**"_p
    // + vfpdef] | "**"_p + vfpdef]] | "*"_p + opt[vfpdef] + (","_p + vfpdef
    // + opt["="_p + E::test]) * _many + opt[","_p + "**"_p + vfpdef] |
    // "**"_p + vfpdef); vfpdef = NAME;
    stmt = simple_stmt | compound_stmt;

    // Simplified small_stmt definition
    simple_stmt = small_stmt + sep(NEWLINE);
    // simple_stmt = printer("simple_stmt") + small_stmt + (";"_p +
    // small_stmt)
    // * _many + opt[";"] + NEWLINE;
    ...
    expr_stmt = converter(fold_assign) <<=
        E::exprlist + ("="_sep + E::exprlist) * _many;
    // expr_stmt = testlist_star_expr + (augassign + (E::yield_expr |
    // E::testlist) |
    //                                   ("="_p + (E::yield_expr |
    //                                   testlist_star_expr)) * _many);
    // augassign = ("+="_p | "-="_p | "*="_p | "@="_p | "/="_p | "%="_p |
    // "&="_p | "|="_p | "^="_p |
    //              "<<="_p | ">>="_p | "**="_p | "//=");
    // // # For normal assignments, additional restrictions enforced by the
    // interpreter del_stmt = "del"_p + E::exprlist;
    ...
    auto dotted_name = converter(unify_many1) <<=
        NAME + ("."_sep + NAME) * _many;
    auto dotted_as_names = converter(unify_many1) <<=
        dotted_as_name + (","_sep + dotted_as_name) * _many;
    auto import_name = "import"_sep + dotted_as_names;
    // ImportStmt is struct but do not use brace_init because funny compile
    // error occurs...
    import_stmt = /* brace_init <<= */ import_name | import_from;

    // Simplified import_as_names definition
    // I think trailing [,] is not necessary
    auto import_as_names = converter(unify_many1) <<=
        import_as_name + (","_sep + import_as_name) * _many;
    // auto import_as_names = converter(unify_many1) <<= import_as_name +
    // (","_sep + import_as_name) * _many + opt[","];
    ...
    const auto normalize_dot_or_elipsis =
        [](const std::variant<Token, Token> &dot_or_elipsis) -> size_t {
    ...
    const auto import_from_from_part =
        [dotted_name, normalize_dot_or_elipsis](
            auto &&token_stream) -> Result<ImportFromFromPart> {
        size_t dot_num = 0;
        auto dot_or_elipsis_parser =
            converter_no_strip(normalize_dot_or_elipsis) <<= "."_p | "..."_p;
        auto first_result = LL1_parse(dot_or_elipsis_parser, token_stream);
        if (first_result.fatally_failed()) {
            return move_to_another_error_result<ImportFromFromPart>(
                first_result.result);
        }
        if (first_result.result) {
            dot_num += first_result.result.success();
            while (true) {
                auto many_dot_or_elipsis_result =
                    LL1_parse(dot_or_elipsis_parser, token_stream);
                if (many_dot_or_elipsis_result.fatally_failed()) {
                    return move_to_another_error_result<ImportFromFromPart>(
                        many_dot_or_elipsis_result.result);
                }
                if (many_dot_or_elipsis_result.result.failed()) {
                    auto dotted_name_result =
                        LL1_parse(dotted_name, token_stream);
                    ...
                    ImportFromFromPart content{
                        dot_num, dotted_name_result.result.extract_success()};
                    return Result<ImportFromFromPart>(content);
                }
    ...
    struct ImportFromImportPart {
        std::variant<ImportFrom::Wildcard, std::vector<ImportAsName>>
            wildcard_or_import_as_names;
        ImportFromImportPart(Token arg)
            : wildcard_or_import_as_names(ImportFrom::Wildcard()) {
            assert(arg == "*");
        }
        ImportFromImportPart(std::vector<ImportAsName> arg)
            : wildcard_or_import_as_names(arg) {}
    };
    ...
    const auto to_import_from = [](const auto &a) -> ImportFrom {
        ...
        return ImportFrom{from.dot_num, from.from_names,
                          import.wildcard_or_import_as_names};
    };
    // # note below = the ('.' | '...') is necessary because '...' is
    // tokenized as ELLIPSIS Do not use converter() insted of
    // converter_no_strip() because funny compile error occurs...
    import_from = converter_no_strip(to_import_from) <<=
        ("from"_sep + import_from_from_part //
         + "import"_sep +
         (result_type<ImportFromImportPart> <<=
          "*"_p | "("_sep + import_as_names + ")"_sep | import_as_names));
    ...
    // with_stmt | funcdef | classdef | decorated | async_stmt; async_stmt =
    // "async"_p + (funcdef | with_stmt | for_stmt);
    if_stmt = result_type<IfStmt> <<= brace_init <<=
        "if"_sep + E::test + ":"_sep + suite +
        (result_type<ast::Elif> <<= brace_init <<=
         "elif"_sep + E::test + ":"_sep + suite) *
            _many +
        opt["else"_sep + ":"_sep + suite];
    // while_stmt = "while"_p + E::test + ":"_p + suite + opt["else"_p +
    // ":"_p + suite];

    // Simplified for_stmt definition, omit else caluse
    for_stmt = result_type<ForStmt> <<= brace_init <<=
        "for"_sep + E::exprlist + "in"_sep + E::testlist + ":"_sep + suite;
    // for_stmt = "for"_sep + E::exprlist + "in"_sep + E::testlist + ":"_sep
    // + suite + opt["else"_sep + ":"_p + suite];

    // try_stmt = ("try"_p + ":"_p + suite + ((except_clause + ":"_p +
    // suite) * _many1 + opt["else"_p + ":"_p + suite] + opt["finally"_p +
    // ":"_p + suite] | "finally"_p + ":"_p + suite)); with_stmt = "with"_p
    // + with_item +
    // (","_p + with_item) * _many + ":"_p + suite; with_item = E::test +
    // opt["as"_p + E::expr];
    // // # NB compile.c makes sure that the default except clause is last
    // except_clause = "except"_p + opt[E::test + opt["as"_p + NAME]];

    suite = simple_stmt |
            sep(NEWLINE) + prompter("(please indent)> ") + sep(INDENT) +
                (stmt + prompter("(in indent)> ")) * _many1_unify + sep(DEDENT);
    ...
    classdef = converter_no_strip(to_class_stmt) <<=
        "class"_sep + NAME +
        flatten_parser(opt["("_sep + opt[E::arglist] + ")"_sep]) + ":"_sep +
        suite;
}
} // namespace ljf::python::grammar
