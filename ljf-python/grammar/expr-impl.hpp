...
namespace detail {
    ...
    static std::variant<std::vector<T>, Comp> fold_to_vector_or_comprehension(
        T t, std::variant<CompFor, std::vector<T>> rest) {
    ...
    static constexpr auto fold_to_exprlist_or_comprehension =
        [](Expr expr, std::variant<CompFor, std::vector<Expr>> rest)
        -> std::variant<ExprList, Comprehension> {
        return fold_to_vector_or_comprehension(std::move(expr),
                                               std::move(rest));
    };

    static constexpr auto make_expr_or_comprehension =
        [](Expr expr, std::optional<CompFor> opt_comp_for)
    ...
    static constexpr auto fold_to_key_datum_list_or_comprehension =
        [](auto key_datum, auto rest) {
            return fold_to_vector_or_comprehension(std::move(key_datum),
                                                   std::move(rest));
        };

    namespace {
        template <typename EnclosureExpr, typename ComprehensionExpr,
                  typename ElemExpr = ast::Expr>
        struct DisplayExprFactory {
            ...
            Expr operator()() const { return EnclosureExpr(); }
        };

        struct ParenthFormExprFactory
            : public DisplayExprFactory<TupleExpr, GeneratorExpr> {
            ...
            Expr operator()(YieldExpr expr) const { return expr; }
        };
    } // namespace

    static constexpr auto parenth_form_conv =
        converter(ParenthFormExprFactory());

    static constexpr auto list_display_conv =
        converter(DisplayExprFactory<ListExpr, ListComprehensionExpr>());
    ...
    // static Expr dict_or_set_display_ctor_impl(std::vector<Expr>
    // expr_list)
    // {
    //     return SetExpr(std::move(expr_list));
    // }

    // static constexpr auto dict_or_set_display_ctor = [](auto &&... args)
    // -> Expr
    // {
    //     return dict_or_set_display_ctor_impl(std::move(args)...);
    // };

    // static constexpr auto dict_or_set_display_conv =
    // converter(dict_or_set_display_ctor);

    static constexpr auto dict_display_conv =
        converter(DisplayExprFactory<DictExpr, DictComprehensionExpr,
                                     DictKeyValueExpr>());

    // string
    static constexpr auto concat_str_exprs =
        [](StringLiteralExpr str_expr,
           std::vector<StringLiteralExpr> rest) -> Result<StringLiteralExpr> {
        if (rest.size() != 0) {
            return make_error(
                rest[0].token(),
                "unsupported feature: string literal concatnation");
        }
        return success_move(str_expr);
    };

    // expr list
    static constexpr auto expr_list_ctor =
        [](Expr first, std::vector<Expr> rest, auto &&opt_comma) -> Expr {
        // matches such like:
        //   a
        //   a + b
    ...
    static constexpr auto combine_tokens = [](Token token1,
                                              Token token2) -> Token {
        auto combined_str = token1.str() + " " + token2.str();
        return Token::create_token<token_category::ANY_OTHER>(
            combined_str, token1.source_location());
    };

} // namespace detail

template <class TokenStream> inline ExprGrammars<TokenStream>::ExprGrammars() {
    ...
    test = (result_type<ConditionalExpr> <<=
            or_test + opt["if"_sep + or_test + "else"_sep + test]) |
           lambdef;
    test_nocond = or_test | lambdef_nocond;
    auto to_lambda_dummy = [](auto &&, Expr) { return LambdaExpr(); };
    lambdef = converter(to_lambda_dummy) <<=
        "lambda"_sep + opt[varargslist] + ":"_sep + test;
    lambdef_nocond = converter(to_lambda_dummy) <<=
        "lambda"_sep + opt[varargslist] + ":"_sep + test_nocond;
    ...
    // for the # sake of a __future__ import described in PEP 401 (which
    // really works :-)
    const Parser comp_op = "<"_p | ">"_p | "=="_p | ">="_p | "<="_p | "<>"_p |
                           "!="_p                                       //
                           | "in"_p                                     //
    ...
    and_expr = converter(fold_left) <<=
        shift_expr + ("&"_p + shift_expr) * _many;
    shift_expr = converter(fold_left) <<=
        arith_expr + (("<<"_p | ">>") + arith_expr) * _many;
    arith_expr = converter(fold_left) <<= term + (("+"_p | "-") + term) * _many;
    term = converter(fold_left) <<=
        factor + (("*"_p | "@"_p | "/"_p | "%"_p | "//") + factor) * _many;
    factor = (result_type<UnaryExpr> <<=
              (result_type<Token> <<= "+"_p | "-"_p | "~") + factor) |
             power;
    power = converter(fold_left_opt) <<= atom_expr + opt["**"_p + factor];

    arglist = converter(fold_left_to_vec) <<=
        argument + (","_sep + argument) * _many + sep(opt[","]);
    const Parser subscriptlist = converter(fold_left_to_vec) <<=
        subscript + (","_sep + subscript) * _many + sep(opt[","]);
    const Parser trailer = result_type<Trailer> <<=
        "("_sep + opt[arglist] + ")"_sep    //
        | "["_sep + subscriptlist + "]"_sep //
        | (result_type<DotIdentifier> <<= "."_sep + NAME);

    const Parser testlist_comp =
        converter(fold_to_exprlist_or_comprehension) <<=
        (result_type<Expr> <<= test | star_expr) +
        (comp_for |
         (","_sep + (result_type<Expr> <<= test | star_expr)) * _many +
             sep(opt[","]));
    atom_expr = result_type<AtomExpr> <<= opt["await"] + atom + trailer * _many;

    atom =
        ((parenth_form_conv <<=
          "("_sep + opt[yield_expr | testlist_comp] + ")"_sep)            //
         | (list_display_conv <<= "["_sep + opt[testlist_comp] + "]"_sep) //
         | (converter_no_strip(default_type<DictExpr>) <<=
            "{"_sep + opt[dictorsetmaker] + "}"_sep)                         //
         | NAME | NUMBER | (converter(concat_str_exprs) <<= STRING * _many1) //
         | (result_type<BuiltinObjectExpr> <<=
            "..."_p | "None"_p | "True"_p | "False"));

    const Parser sliceop = ":"_sep + opt[test];
    subscript = test | (result_type<SliceExpr> <<=
                        opt[test] + ":"_sep + opt[test] + opt[sliceop]);

    const Parser expr_or_star_expr = result_type<Expr> <<= expr | star_expr;
    exprlist = converter(expr_list_ctor) <<=
        expr_or_star_expr + (","_sep + expr_or_star_expr) * _many + opt[","];
    testlist = converter(expr_list_ctor) <<=
        test + (","_sep + test) * _many + opt[","];
    const Parser key_datum = result_type<DictKeyValueExpr> <<=
        test + ":"_sep + test | "**"_p + expr;
    const Parser dict_maker = dict_display_conv <<=
        converter(fold_to_key_datum_list_or_comprehension) <<=
        (key_datum +
         (comp_for | (","_sep + key_datum) * _many + sep(opt[","])));
    ...
    // using NAME # results in an ambiguity. ast.c makes sure it's a NAME. #
    // "test '=' test" is really "keyword '=' test", but we have no such
    // token. # These need to be in a single rule to avoid grammar that is
    // ambiguous # to our LL(1) parser. Even though 'test' includes '*expr'
    // in star_expr, # we explicitly match '*' here, too, to give it proper
    // precedence. # Illegal combinations and orderings are blocked in
    // ast.c: # multiple (test comp_for) arguments are blocked; keyword
    // unpackings # that precede iterable unpackings are blocked; etc.
    argument =
        ((converter(make_expr_or_comprehension) <<= test + opt[comp_for]) |
         (test + "="_p + test)
