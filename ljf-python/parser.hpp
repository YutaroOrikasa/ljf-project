...
// result: Result<T>
// return type: kind of Result<std::optional<T>>
template <typename T> auto to_optional_result(Result<T> &&result) {
    if (result.failed()) {
...
template <typename T> auto to_empty_optional_result(Result<T> &&result) {
    return Result<std::optional<T>>(std::optional<T>());
}

inline constexpr auto option = [](auto parser) {
    return Parser([=](auto &&token_stream) {
        auto initial_pos = token_stream.current_position();
        auto result = parser(token_stream);
        // if result.failed() && token_stream.current_position() ==
        // initial_pos it is not error because this is the part of LL1
        // parser.
...
inline constexpr auto string = [](auto &&str) {
    return read_if([=](const Token &token) { return token.str() == str; },
...
namespace abbrev {
    ...
    template <size_t, bool unify = false> struct Many {};

    template <typename Parser> constexpr auto operator*(Parser &&p, Many<0>) {
...
constexpr auto token(token_category cat) {
    return read_if([=](const Token &token) { return token.category() == cat; },
...
inline constexpr Parser identifier = result_type<ast::IdentifierExpr> <<=
    token(token_category::IDENTIFIER);

inline constexpr Parser string_literal = result_type<ast::StringLiteralExpr> <<=
    read_if([](const Token &token) { return token.is_string_literal(); });
inline constexpr Parser integer_literal =
    result_type<ast::IntegerLiteralExpr> <<=
    read_if([](const Token &token) { return token.is_integer_literal(); });
