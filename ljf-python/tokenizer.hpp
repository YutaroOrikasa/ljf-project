...
public:
    ...
    size_t indent_width() const noexcept { return indent_.size(); }
};
...
public:
    ...
    template <
        typename T,
        std::enable_if_t<std::is_constructible_v<variant_type, T>> * = nullptr>
...
template <typename IStream, bool discard_empty_line> class TokenStream {
private:
    ...
    std::vector<Nester> nester_stack_{
        detail::tokenizer::phase2::IndentNester("")};
...
public:
    ...
    std::size_t current_position() const noexcept { return current_position_; }
        ...
        while (token_buffer_.empty()) {
            ...
            if (token.category() == token_category::NEWLINE) {
                ...
                if (is_prev_token_newline()) {
                    assert(!discard_empty_line &&
                           "consecutive newline tokens must not exist when "
                           "discard_empty_line=true");
...
using IStreamTokenStream =
    TokenStream<std::istream, /*discard_empty_line=*/false>;
using FStreamTokenStream =
    TokenStream<std::fstream, /*discard_empty_line=*/true>;
using SStreamTokenStream =
    TokenStream<std::stringstream, /*discard_empty_line=*/true>;
