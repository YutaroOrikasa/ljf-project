...
static auto parse_if(bool cond, const Parser &p, TokenStream &stream,
                     ErrArgs &&...err_args) {
    using result_content_ty = result_content_t<Parser, TokenStream>;
    if (cond) {
        return p(stream);
    } else {
        return make_error_result<result_content_ty>(
            stream.peek(), std::forward<ErrArgs>(err_args)...);
    }
}

template <typename Parser, typename TokenStream>
static auto parse_until_eof(const Parser &p, TokenStream &stream) {
    auto parser = separator(many(newline)) + p + separator(many(newline)) +
                  separator(eof);
...
static auto parse_until_eof(const parser::Parser<TResult, F> &p,
                            TokenStream &stream) {
    auto parser = separator(many(newline)) + p + separator(many(newline)) +
                  separator(eof);
